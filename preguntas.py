...
def pregunta_11():
    ...
    39   39    a,d,f
    """
    dataf = pd.DataFrame({'_c2': (tbl1.groupby('_c0')['_c4'])})
    dataf1 = pd.DataFrame({'_c0': dataf['_c2'].map(lambda x: x[0])})
    dataf2 = pd.DataFrame({'_c4': dataf['_c2'].map(lambda x: ','.join(sorted(x[1])))})
...
def pregunta_12():
    ...
    """
    dataf0 = pd.DataFrame({'join': tbl2['_c5a'] + ':' + tbl2['_c5b'].astype(str)})
    dataf1 = pd.concat([pd.DataFrame({'_c0': tbl2['_c0']}), dataf0],axis=1)

    dataf = pd.DataFrame({'_c2': (dataf1.groupby('_c0')['join'])})
    datafx = pd.DataFrame({'_c0': dataf['_c2'].map(lambda x: x[0])})
    datafy = pd.DataFrame({'_c5': dataf['_c2'].map(lambda x: ','.join(sorted(x[1])))})
...
def pregunta_13():
    ...
    tbl0['_c5b'] = tbl2.groupby(['_c0'])['_c5b'].sum()
